...
export class PlacesService {
  ...
  loadUserPlaces() {
    return this.fetchPlaces('http://localhost:3000/user-places', 'Something went wrong fetching the available places. Please try again later.').pipe(tap({
  ...
  addPlaceToUserPlaces(place : Place) {
    ...
    return this.httpClient.put('http://localhost:3000/user-places', {
      ...
      catchError(error => {
        ...
        this.errorService.showError('Failed to save the selected place.');
        return throwError(() => new Error('Failed to save the selected place.'));
      })
    )
